...
namespace KitOnline.Models
{
    public class Check
    {
        ...
        {
            if (Subjects.Count == 0)
                throw new Exception("Невозможно скорректировать цены, т.к. в чеке нет ни одного товара");
            ...
            var oneSubject = Subjects.OrderByDescending(it => it.Price).FirstOrDefault(subject =>
                subject.Quantity == 1m && subject.Price > Math.Abs(difference));
            if (oneSubject != null)
            {
            ...
            maxPriceSubject!.Price += (long)(difference / maxPriceSubject.Quantity);
